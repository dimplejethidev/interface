...
const TutorialChecklistPopup = () => {
    ...
    return (
        ...
        >
            ...
            <div className={'transition-all delay-100 duration-300 overflow-hidden space-y-4 ' + (isCollapsed ? ' max-h-0 max-w-0 ' : ' max-h-64 max-w-xs pt-4 ')}>
                <ChecklistItem
                    text='Connect wallet'
                    itemState={tutorialContext?.connectedWallet}
                    setItemState={tutorialContext?.setConnectedWallet}
                />
                <ChecklistItem
                    text='Request funds'
                    itemState={tutorialContext?.requestedPay}
                    setItemState={tutorialContext?.setRequestedPay}
                />
                <ChecklistItem
                    text='Start a swap'
                    itemState={tutorialContext?.startedSwap}
                    setItemState={tutorialContext?.setStartedSwap}
                />
            </div>
        </div >
    )
